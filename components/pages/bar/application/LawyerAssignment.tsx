...
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { ILawyer } from '@/types/lawyer'; // Tip dosyasından Lawyer tipi
import { mockLawyers } from '@/utils/mockData'; // Mock data import

interface LawyerAssignmentProps {
  onAssign: (lawyer: ILawyer) => void;
}

export default function LawyerAssignment({ onAssign }: LawyerAssignmentProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [filteredLawyers, setFilteredLawyers] = useState<ILawyer[]>(mockLawyers);
